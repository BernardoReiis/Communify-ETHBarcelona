...
async def login_metamask(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if message.chat.type == "private":
        buttons = [
            InlineKeyboardButton("Communify Homepage", callback_data="homepage"),
            InlineKeyboardButton("Create Quest", callback_data="create_quest"),
            InlineKeyboardButton("Quests", callback_data="quests"),
            InlineKeyboardButton("Create Donation Campaign", callback_data="create_donation_campaign"),
    ...
    await update.message.reply_text(
        "Please press the button to login with Metamask",
        reply_markup=ReplyKeyboardMarkup.from_button(
            KeyboardButton(
                text="Please login with Metamask",
                web_app=WebAppInfo(url="https://communify-to-demo.vercel.app/"),
            )
        ),
    )
    ...
    if message.chat.type == "private":
        await update.message.reply_text(
            ...
            reply_markup=ReplyKeyboardMarkup.from_button(
                KeyboardButton(
                    text="Donations",
                    web_app=WebAppInfo(url="https://communify-to-demo.vercel.app/#donations"),
                )
            ),
        )

async def create_donation_campaign(update, context):
    message = update.message
    if message.chat.type == "private":
        await update.message.reply_text(
            "Please press the button to create a donation campaign",
            reply_markup=ReplyKeyboardMarkup.from_button(
                KeyboardButton(
...
async def homepage(update, context):
    message = update.message
    if message.chat.type == "private":
        await update.message.reply_text(
            "Communify Homepage",
...
async def button_callback(update, context):
    ...
    elif command == "create_donation_campaign":
        await context.bot.send_message(chat_id=chat_id, text="/create_donation_campaign")
    elif command == "homepage":
        await context.bot.send_message(chat_id=chat_id, text="/homepage")
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("create_donation_campaign", create_donation_campaign))
